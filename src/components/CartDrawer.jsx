      ...
      {isCartOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed right-0 top-0 bottom-0 z-50 w-full max-w-md bg-white dark:bg-zinc-950 shadow-2xl overflow-hidden flex flex-col justify-between border-l dark:border-zinc-900"
            data-lenis-prevent
          >
            {/* Header */}
            <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 dark:border-zinc-900 bg-[#FAF9F6] dark:bg-zinc-900/40">
              <div className="flex items-center gap-2">
                <FiShoppingBag className="text-primary text-xl" />
                <h3 className="font-display font-black text-lg text-text-dark dark:text-gray-100">Your Cart</h3>
                {cartItems.length > 0 && (
              ...
              <button
                onClick={handleClose}
                className="text-text-muted dark:text-gray-400 hover:text-text-dark dark:hover:text-gray-100 cursor-pointer p-1.5 hover:bg-gray-100 dark:hover:bg-zinc-900 rounded-full transition-colors focus:outline-none"
              >
            ...
            <div className="flex-1 overflow-y-auto p-6 scrollbar-thin">
                ...
                <>
                  {cartItems.length === 0 ? (
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-center py-12">
                      ...
                      <h4 className="font-display font-black text-lg text-text-dark dark:text-gray-100 mb-1">Your cart is empty</h4>
                      <p className="text-xs text-text-muted dark:text-gray-400 max-w-xs mb-6">
                    ...
                    <div className="flex flex-col gap-4">
                      {cartItems.map((item) => (
                        <div
                          ...
                          className="flex items-center gap-4 bg-white dark:bg-zinc-900/60 border border-gray-100 dark:border-zinc-800 p-3 rounded-2xl shadow-xs relative group hover:border-primary/10 transition-all"
                        >
                          <img
                            src={item.image}
                            alt={item.name}
                          ...
                          <div className="flex-1 min-w-0 text-left">
                            <h4 className="font-display font-extrabold text-xs text-text-dark dark:text-gray-100 truncate">
                              {item.name}
                            </h4>
                            <p className="text-[10px] text-text-muted dark:text-gray-400 font-semibold truncate mt-0.5">
                              {item.origin}
                            </p>
                            <span className="inline-block bg-[#e8f5e9] dark:bg-emerald-950/40 text-[#2e7d32] dark:text-emerald-400 text-[8px] font-black uppercase tracking-wider px-2 py-0.5 rounded mt-1 border border-[#c8e6c9] dark:border-emerald-900/30">
                              {item.potType}
                            </span>
                            <div className="flex items-center justify-between mt-3">
                              {/* Quantity Selector */}
                              <div className="flex items-center border border-gray-200 dark:border-zinc-800 rounded-lg p-0.5 bg-gray-50/50 dark:bg-zinc-900/80">
                                <button
                                  onClick={() => updateQuantity(item.id, item.potType, item.quantity - 1)}
                                  className="p-1 text-text-muted dark:text-gray-400 hover:text-text-dark dark:hover:text-gray-100 cursor-pointer hover:bg-gray-100 dark:hover:bg-zinc-800 rounded"
                                >
                                  <FiMinus className="text-[10px]" />
                                </button>
                                <span className="px-2 text-xs font-black text-text-dark dark:text-gray-100 w-6 text-center">
                                  {item.quantity}
                                </span>
                                <button
                                  onClick={() => updateQuantity(item.id, item.potType, item.quantity + 1)}
                              ...
                              <div className="text-right">
                                <span className="text-[10px] text-text-muted dark:text-gray-400 block">₹{item.price} each</span>
              ...
              {checkoutStep === 'details' && (
                ...
                <form onSubmit={handleCheckoutSubmit} className="flex flex-col gap-4 text-left">
                  <div className="mb-2">
                    ...
                    <h3 className="font-display font-black text-xl text-text-dark dark:text-gray-100 mt-2">
                  ...
                  <div className="flex flex-col gap-1">
                    <label className="text-xs font-bold text-text-dark dark:text-gray-200">Phone Number</label>
                    <input
                      type="tel"
                      required
                  ...
                  <div className="flex flex-col gap-1">
                    <label className="text-xs font-bold text-text-dark dark:text-gray-200">Delivery Address</label>
                    <textarea
                      rows="3"
                      required
                  ...
                  <div className="flex flex-col gap-1">
                    <label className="text-xs font-bold text-text-dark dark:text-gray-200">Custom Message (Optional)</label>
                    <textarea
                      rows="2"
                  ...
                  <button
                    ...
                    className="w-full py-2 bg-gray-50 dark:bg-zinc-900 text-text-muted dark:text-gray-400 hover:text-text-dark dark:hover:text-gray-100 font-bold text-xs rounded-xl border border-gray-100 dark:border-zinc-800 transition-colors focus:outline-none cursor-pointer"
                  >
                    Back to Cart
                  </button>
                </form>
              ...
              {checkoutStep === 'success' && (
                ...
                <div className="text-center py-8">
                  <div className="w-16 h-16 bg-green-50 dark:bg-emerald-950/40 text-primary border border-green-100 dark:border-emerald-900/30 rounded-full flex items-center justify-center mx-auto mb-4 animate-bounce">
                    <FiCheckCircle className="text-3xl" />
                  </div>
                  <h3 className="font-display font-black text-2xl text-text-dark dark:text-gray-100 mb-2">Order Confirmed!</h3>
                  <p className="text-sm text-text-muted dark:text-gray-400 max-w-sm mx-auto mb-6 leading-relaxed">
                    Thank you <span className="font-bold text-text-dark dark:text-gray-100">{shippingForm.name}</span>. Your order has been placed. The accredited growers will contact you on <span className="font-bold text-text-dark dark:text-gray-100">{shippingForm.phone}</span> for shipping verification.
                  </p>

                  <div className="bg-primary/5 dark:bg-primary/10 border border-primary/10 dark:border-primary/20 rounded-2xl p-4 mb-6 text-left">
                    <span className="text-xs font-bold text-primary block mb-1">Want instant updates and live tracking?</span>
                    <p className="text-[11px] text-text-muted dark:text-gray-300">Download the GeoTree Mart App to directly communicate with growers and verify nursery logistics.</p>
                  </div>
            ...
            {cartItems.length > 0 && checkoutStep === 'cart' && (
              <div className="border-t border-gray-100 dark:border-zinc-900 p-6 bg-[#FAF9F6] dark:bg-zinc-900/40 flex flex-col gap-4 text-left">
                {/* Free Shipping Progress bar */}
                <div>
                  <div className="flex justify-between text-[10px] font-bold text-text-dark dark:text-gray-250 mb-1">
                    <span>
                ...
                <div className="flex flex-col gap-1.5 text-xs text-text-dark">
                  <div className="flex justify-between">
                    <span className="font-medium text-text-muted dark:text-gray-400">Cart Subtotal</span>
                    <span className="font-bold dark:text-gray-100">₹{cartTotal.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="font-medium text-text-muted dark:text-gray-400">Shipping Fee</span>
                  ...
                  <div className="flex justify-between text-sm">
                    <span className="font-display font-black dark:text-gray-100">Grand Total</span>
                    <span className="font-display font-black text-primary text-base font-extrabold">₹{grandTotal.toFixed(2)}</span>
                  </div>
                </div>
